...
exports.getOneUser = (req, res) => {
  ...
  db.query("SELECT user.nom, user.prenom, user.email, user.user_imageUrl, post.id, post.post_content, post.post_imageUrl, post.post_likes, post.post_date, post.user_id, comment.comment_content FROM user LEFT JOIN post ON user.id = post.user_id LEFT JOIN comment ON user.id = comment.user_id WHERE user.id = ? ORDER BY post.post_date DESC", [id], (error, result) => {
    if (!error) {
      if (result.length == 0) {
